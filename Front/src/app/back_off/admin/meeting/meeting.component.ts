...
export class MeetingComponent implements OnInit {
  ...
  startMeeting(): void {
    ...
    selectedUsers.forEach(user => {
      this.sendMeetingEmail(user.email, roomID, meetingUrl).subscribe({
        ...
        error: (err) => {
          console.error(`Error sending meeting email to ${user.email}:`, err);
          this.errorMessage = 'Failed to send some meeting invitations. Check console for details.';
        }
      });
    });
  ...
  private sendMeetingEmail(email: string, roomID: string, meetingUrl: string) {
    const emailRequest = {
      ...
      body: `You have been invited to a meeting. Join using this link: ${meetingUrl}\nMeeting ID: ${roomID}`
    };
