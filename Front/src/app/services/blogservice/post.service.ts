...
export class PostService {
  ...
  updatePost(postId: number, postData: any): Observable<any> {
    return this.http.put(`http://localhost:8081/api/posts/${postId}`, postData)

  }
  deletePost(postId: number) {
    return this.http.delete(`http://localhost:8081/api/posts/${postId}`, { responseType: 'text' });
  }
  ...
  getSummary(postId: number): Observable<string> {
    return this.http.get(BASIC_URL + `api/posts/${postId}/summary`, { responseType: 'text' });
  }
